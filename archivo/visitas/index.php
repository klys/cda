...
<table class="table table-striped">
  ...
  <?php while($row = $query->fetch_assoc()) { $sid = $row["ide"]; ?>
  <tr>
  	<td> <?= $row["visitante"] ?></td>
  	<td> <?= $row["visitado"] ?></td>
    <td> <?= $row["lugar"] ?></td>
    <td> <?= $row["fin"] ?></td>
    <td> <?php if ($row["fout"] == "") { echo "<button class = 'btn btn-danger' onclick = 'salida_form({$sid})'>Asignar</button>";} else echo $row["fout"]; ?></td>
  </tr>
  <?php } ?>
